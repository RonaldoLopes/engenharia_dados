...
SELECT DISTINCT
  ...
  load_ts,
  "mongodb" AS source
FROM STREAM(live.raw_vw_py_mondodb_user)
...
(
  ...
  load_ts TIMESTAMP,
  source STRING
  CONSTRAINT valid_hk_vehicle_id EXPECT(hk_vehicle_id IS NOT NOT NULL) ON VIOLATION DROP ROW,
...
(
  ...
  load_ts TIMESTAMP,
  source STRING
  CONSTRAINT valid_hk_txn_id EXPECT(hk_txn_id IS NOT NOT NULL) ON VIOLATION DROP ROW,
...
(
  ...
  load_ts TIMESTAMP,
  source STRING
  CONSTRAINT valid_hk_subscription_id EXPECT(hk_subscription_id IS NOT NOT NULL) ON VIOLATION DROP ROW,
  CONSTRAINT valid_id EXPECT(id IS NOT NULL) ON VIOLATION DROP ROW
)
AS
SELECT DISTINCT
  ...
  load_ts,
  "postgres" AS source
FROM STREAM(live.raw_vw_py_postgres_subscriptions)
