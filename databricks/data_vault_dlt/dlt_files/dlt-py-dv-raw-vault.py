...
# MAGIC %md
# MAGIC # Raw Vault [Python]
# MAGIC
# MAGIC * raw_vw_py_mssql_user
# MAGIC * raw_vw_py_mongodb_user
# MAGIC * raw_vw_py_postgres_vehicle
# MAGIC * raw_vw_py_postgres_payments
# MAGIC * raw_vw_py_postgres_subscription
# MAGIC * raw_vw_py_mongodb_rides
...
# DBTITLE 1,raw_vw_py_mongodb_user
@dlt.view()
def raw_vw_py_mongodb_user():
...
def raw_vw_py_postgres_vehicle():
  ...
  return (
    dlt.readStream("bronze_postgres_vehicle").select(
        ...
        expr(
            "CASE WHEN year IN ('2018', '2019', '2020') THEN 'Current' " + "WHEN year IN ('2013', '2014', '2015', '2016', '2017') THEN 'Recent' WHEN year IN ('2003', '2004', '2005', '2006', '2007', '2008', '2009', '2010', '2011', '2012') THEN 'Mid-Age' WHEN year < 2002 THEN 'Old'" + "ELSE 'Not-Classified' END"
...
def raw_vw_py_postgres_payments():
  ...
  return (
    dlt.readStream("bronze_postgres_payments").select(
...
def raw_vw_py_postgres_subscription():
  ...
  return (
    dlt.readStream("bronze_postgres_subscription").select(
...
def raw_vw_py_mongodb_rides():
  ...
  return (
    dlt.readStream("bronze_mongodb_rides").select(
        lit(current_timestamp()).alias("load_ts"),
        lit("mongodb").alias("source"), 
        col("id").alias("id"),
        col("user_id").alias("user_id"),
